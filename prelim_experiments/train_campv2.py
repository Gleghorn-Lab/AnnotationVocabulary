...
from udev_public.models.camp.modeling_campv2 import CAMPv2
from udev_public.models.camp.config_campv2 import CAMPv2Config
...
class CampCollator:
    ...
    def __call__(self, examples):
        ...
        target_input_ids, _ = self._mask_tokens(
            inputs=original_ids.clone(),
            labels=original_ids.clone(),
            vocab_size=self.seq_vocab_size,
            mask_prob=self.target_mask_probability,
            mask_token_id=self.seq_mask_token_id,
            special_token_ids=self.seq_special_token_ids,
            seq=True
        )
        # target labels include nonmasked tokens
...
def get_args():
    ...
    parser.add_argument('--context_mask', type=float, default=0.70)
    parser.add_argument('--target_mask', type=float, default=0.10)
    parser.add_argument('--ann_mask', type=float, default=0.10)
    ...
    parser.add_argument('--contrastive_loss', type=str, default='space')
...
def main():
    ...
    data_collator = CampCollator(
        ...
        target_mask_probability=args.target_mask,
